export default async function handler(req, res) {
  ...
  // 71400–71421 urban planning & landscape architecture → Urban Designer + Landscape Architect chips
  const CPV_URBAN_LANDSCAPE = new Set([
    '71400000','71410000','71411000','71412000','71413000',
    '71420000','71421000',
  ]);
  ...
  const ALL_CPV = new Set([
    ...CPV_DEVELOPER, ...CPV_ARCHITECT, ...CPV_URBAN_LANDSCAPE,
    ...CPV_ENGINEER, ...CPV_CONSTRUCTION,
  ]);

  // Broad prefix matching for sub-codes not explicitly listed.
  // Tightly scoped to avoid pulling in unrelated 711xx (surveying/inspection) noise.
  const CPV_PREFIXES = [
  ...
  const DEVELOPER_KEYWORDS = [
    ...
    'invitation to tender','invitation to negotiate',
  ...
  // Keywords specific to urban design / landscape architecture chips
  const URBAN_LANDSCAPE_KEYWORDS = [
    'urban design','urban designer',
    'placemaking','public realm',
    'landscape design','landscape architect','landscape architecture',
    'landscape consultant','open space design',
  ...
  const ALL_KEYWORDS = [
    ...
    ...URBAN_LANDSCAPE_KEYWORDS, ...ENGINEERING_KEYWORDS,
  ];
  ...
  // Returns chip-filter value matching index.html setProf() values
  function cpvProfession(id) {
    if (!id) return null;
    if (CPV_DEVELOPER.has(id))       return 'developer';
    if (CPV_ARCHITECT.has(id))       return 'architect';
    if (CPV_URBAN_LANDSCAPE.has(id)) return 'urban';      // matches both Urban Designer + Landscape Architect chips
    ...
    if (id.startsWith('7140') || id.startsWith('7141') || id.startsWith('7142'))                       return 'urban';
    return null;
  }
  ...
  function keywordProfession(title, fullText) {
    ...
    if (URBAN_LANDSCAPE_KEYWORDS.some(kw => t.includes(kw) || f.includes(kw))) return 'urban';
  ...
  function isRelevant(release) {
    ...
    // Trusted CPVs → always include without needing keyword check
    if (allCpvs.some(id =>
      CPV_DEVELOPER.has(id) || CPV_ARCHITECT.has(id) || CPV_URBAN_LANDSCAPE.has(id)
    )) return true;
